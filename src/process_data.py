...
class DBpediaProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        examples = []
        label_file = open(os.path.join(data_dir, f"{split}_labels.txt"), 'r', encoding='utf-8')
...
class ImdbProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        ...
        MAX_CHARS = 5000  # hoặc 1500–3000 tùy dung lượng GPU/model

        # Danh sách để lưu độ dài ký tự của các câu
        text_lengths = []
        ...
        with open(os.path.join(data_dir, f'{split}.txt'), 'r', encoding="utf-8") as fin:
            for idx, line in enumerate(fin):
                ...
                # Tính độ dài ký tự của text_a và lưu vào danh sách
                text_lengths.append(len(text_a))

                example = InputExample(guid=str(idx), text_a=text_a, label=int(labels[idx]))
                examples.append(example)

        # Tính toán và in số lượng ký tự dài nhất và trung bình
        if text_lengths:
            max_length = max(text_lengths)
            avg_length = sum(text_lengths) / len(text_lengths)
            print(f"IMDB {split} dataset:")
            print(f"  Longest sentence: {max_length} characters")
    ...
    def get_test_labels_only(data_dir, dirname):
        label_file = open(os.path.join(data_dir, dirname, "test_labels.txt"), 'r', encoding="utf-8") 
        labels  = [int(x.strip()) for x in label_file.readlines()]
        return labels
